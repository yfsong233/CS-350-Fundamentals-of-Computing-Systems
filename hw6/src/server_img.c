...
// struct image* images[MAX_IMG]; 
struct LL * list = NULL;
uint64_t numof_img = -1;
int numof_items = 0;
...
uint32_t add_image(struct image * img) {
    numof_img++;
...
// struct image * get_image(uint32_t id) {
//     if (id < numof_img) {
//         return images[id];
//     }
//     return NULL;  
...
void replace_image (struct image * img, int num){
	struct image * old = get_image(num);
	old->width = img->width;
	old->height = img->height;
	old->pixels = img->pixels;
}
...
int worker_main (void * arg)
{
	...
	int thread_id = params->worker_id;
	...
	while (!params->worker_done) {
		...
		if (req.request.img_op != IMG_RETRIEVE && req.request.img_op != IMG_UNUSED){
			struct image * img = get_image(((int) req.request.img_id));
			switch (req.request.img_op){
				case IMG_ROT90CLKW:
					img = rotate90Clockwise(img, NULL);
					if (req.request.overwrite == 1){
						replace_image (img, (int)req.request.img_id);
						resp.img_id = req.request.img_id; 
					}
					else{
						add_image(img);
						resp.img_id = numof_img;
					}
					break;
				case IMG_BLUR:
					img = blurImage(img);
			...
			sync_printf("T%d R%ld:%lf,%s,%u,%lu,%lu,%lf,%lf,%lf\n", thread_id, req.request.req_id, 
			...
			resp.ack = RESP_COMPLETED;
			send(params->conn_socket, &resp, sizeof(struct response), 0);
			sendImage(get_image((int) resp.img_id), params->conn_socket);
			clock_gettime(CLOCK_MONOTONIC, &req.completion_timestamp);		
	...
	do {
		...
		if (in_bytes > 0) {
			...
			if(req->request.img_op == IMG_REGISTER){
				...
				sync_printf("T%d R%ld:%lf,%s,%u,%lu,%lu,%lf,%lf,%lf\n", 
				thread_id, req->request.req_id, TSPEC_TO_DOUBLE(req->request.req_timestamp),
				OPCODE_TO_STRING(req->request.img_op), 
				req->request.overwrite,
				req->request.img_id,
				resp.img_id,
				...
				// 	resp.ack = RESP_COMPLETED;
				// 	send(conn_socket, &resp, sizeof(struct response), 0);
				// 	sendImage(get_image((int) resp.img_id), conn_socket);
				// }
