...
int worker_main(void* arg) {
    ...
    while(1) {
        struct request current_req = get_from_queue(wrk->q);
        clock_gettime(CLOCK_MONOTONIC, &start_time);

		get_elapsed_busywait(current_req.req_length.tv_sec, current_req.req_length.tv_nsec);
...
void handle_connection(int conn_socket)
{
    ...
    the_queue->count = 0;
    struct timespec receipt_time;
    struct worker_params* wrk = malloc(sizeof(struct worker_params));
    if (!wrk) {
	...
	do {
		...
		// printf("now adding req to queue");
